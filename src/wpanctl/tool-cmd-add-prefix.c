...
int tool_cmd_add_prefix(int argc, char* argv[])
{
	...
	dbus_bool_t is_domain_prefix = FALSE;
	...
	while (1) {
		...
		c = getopt_long(argc, argv, "ht:P:l:sfadcroD", long_options, &option_index);
	...
	dbus_message_append_args(
		...
		DBUS_TYPE_BOOLEAN, &is_domain_prefix,
		DBUS_TYPE_INVALID
	);
	...
	if (ret == 0) {
		...
		fprintf(
			stderr,
			"Successfully added prefix \"%s\" len:%d stable:%c [on-mesh:%c def-route:%c config:%c dhcp:%c slaac:%c pref:%c prio:%s domain:%c]\n",
			address_string,
			prefix_len_in_bits,
			stable ? '1' : '0',
			on_mesh ? '1' : '0',
